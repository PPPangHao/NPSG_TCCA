...
class MAHNOBChunkNPYDataset(Dataset):
    def __init__(self,
        ...
        for sess_key in self.predictions.keys():
            ...
            # 你的 Arousal 划分标准：
            # Class 0 (Resting): < 3
            # Class 1 (Mid Arousal): 3 <= x <= 5
            # Class 2 (High Arousal): > 5
        ...
        print(f"Class Distribution: Class 0 (Resting <3): {class_counts[0]}, "
              f"Class 1 (Mid 3-5): {class_counts[1]}, "
              f"Class 2 (High >5): {class_counts[2]}")
        
        self.label_map = {0: "Resting (<3)", 1: "Mid (3-5)", 2: "High (>5)"}
...
def train_multimodal_kfold(
    ...
    cm_dir = os.path.join(output_dir, "confusion_matrices_3cls")
    ...
    for fold, (train_sids_idx, val_sids_idx) in enumerate(kf.split(unique_sids)):
        ...
        # 修改点：num_classes = 3
        multimodal_model = MultiModalTCMAClassification(num_classes=3, dropout=0.3)
        ...
        if resume_path and os.path.exists(resume_path):
            ...
            # 注意：如果 resume 的权重是 2 分类的，这里可能会报错，建议只加载部分权重或从头训练
            try:
                multimodal_model.load_state_dict(ckpt["model_state_dict"], strict=False)
                print("Loaded weights successfully (strict=False).")
            except Exception as e:
...
if __name__ == "__main__":
    ...
    # 路径更新为 3cls
    SAVE_LAST_PATH = "./output_TCCA_arousal_3cls/multimodal_tcma_mahnob_last_3cls.pth"
    SAVE_BEST_PATH = "./output_TCCA_arousal_3cls/multimodal_tcma_mahnob_best_3cls.pth"
    
    # 注意：如果之前的权重是 5分类 或 2分类，加载可能会有 shape mismatch，建议设为 None 从头训练
    # 或者你需要手动剔除最后的全连接层权重
    RESUME_PATH = None 
